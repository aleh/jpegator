...
#Region " Windows Form Designer generated code "
    ...
    Private Sub InitializeComponent()
        ...
        Me.TextBox.Size = New System.Drawing.Size(176, 32)
        ...
        Me.SaveFileDialog.InitialDirectory = "\My Documents\My Pictures"
...
#End Region
    ...
    Private Sub DrawLineEx(ByVal g As Graphics, ByVal color As Color, ByVal Width As Integer, ByVal p1 As Point, ByVal p2 As Point)
        ...
        Dim l As Single = Math.Sqrt(dx * dx + dy * dy)
        If (Math.Abs(l) > 0.01) Then
            dx = dx * Width / (2 * l)
            dy = dy * Width / (2 * l)
            Dim polygon As Point() = { _
                New Point(Math.Round(p1.X - dy), Math.Round(p1.Y + dx)), _
                New Point(Math.Round(p1.X + dy), Math.Round(p1.Y - dx)), _
                New Point(Math.Round(p2.X + dy), Math.Round(p2.Y - dx)), _
                New Point(Math.Round(p2.X - dy), Math.Round(p2.Y + dx)) _
            }
            g.FillPolygon(New SolidBrush(color), polygon)
        End If
        Dim w2 As Single = Width / 2
        g.FillEllipse(New SolidBrush(color), Math.Round(p1.X - w2), Math.Round(p1.Y - w2), Width, Width)
        g.FillEllipse(New SolidBrush(color), Math.Round(p2.X - w2), Math.Round(p2.Y - w2), Width, Width)
    End Sub
    ...
    Private Sub PictureBox_MouseMove(ByVal sender As Object, ByVal e As MouseEventArgs) Handles PictureBox.MouseMove
        ...
        If PencilMenuItem.Checked And Drawing Then
            ...
            Dim left = Math.Min(LastPencilPos.X - PencilWidth, e.X)
            Dim right = Math.Max(LastPencilPos.X + PencilWidth, e.X)
            Dim top = Math.Min(LastPencilPos.Y - PencilWidth, e.Y)
            Dim bottom = Math.Max(LastPencilPos.Y + PencilWidth, e.Y)
    ...
    Private Sub TextEditBox_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles TextBox.LostFocus
        ...
        If Editing Then
            ...
            If TextBox.Modified Then
                ...
                Dim x = TextBox.Left - PictureBox.Left
                Dim y = TextBox.Top - PictureBox.Top
                ...
                Dim Rect As New Rectangle(x, y, TextSize.Width, TextSize.Height)
            ...
            ' Hide text box and move to upper-left corner, so it does not affect scrollbars
